...
function listCurrencieshiden(lim) {
	jQuery("#fakturo-currencies-box").find('div.inside table tbody tr').each(function(i) {
		/*hidden element*/
		if (i >= lim) { jQuery(this).hide(); }
		else { jQuery(this).show(500); }
	});
...
function productQuantityEvent(element) {
	...
	jQuery(element).data('focused', true);
}
...
function updateKeyPress() {
	...
	jQuery('.product_quality').on('keypress', function(e) {
		if (e.which == 13) {
			productQuantityEvent(this);
			e.preventDefault();
			return false;
		}
	});
	jQuery('.product_quality').on('change', function(e) {
		productQuantityEvent(this);
	});
